...
func (d *DB) GetLinkBySlug(slug string) (*Link, error) {
	row := d.Conn.QueryRow("SELECT id, slug, url, item_id, created_by, clicks, created_at FROM links WHERE slug = ?", slug)
	return scanLink(row)
}

func (d *DB) GetLinkByItemID(itemID int64) (*Link, error) {
	row := d.Conn.QueryRow("SELECT id, slug, url, item_id, created_by, clicks, created_at FROM links WHERE item_id = ?", itemID)
	return scanLink(row)
}
...
func (d *DB) ListLinks() ([]Link, error) {
	rows, err := d.Conn.Query("SELECT id, slug, url, item_id, created_by, clicks, created_at FROM links ORDER BY id DESC")
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		var ts string
		if err := rows.Scan(&l.ID, &l.Slug, &l.URL, &itemID, &l.CreatedBy, &l.Clicks, &ts); err != nil {
			return nil, err
		}
		l.ItemID = itemID
...
func scanLink(row interface {
	...
	var itemID sql.NullInt64
	var ts string
	if err := row.Scan(&l.ID, &l.Slug, &l.URL, &itemID, &l.CreatedBy, &l.Clicks, &ts); err != nil {
		return nil, err
	}
	l.ItemID = itemID
	l.CreatedAt, _ = time.Parse("2006-01-02 15:04:05", ts)
